...
Please refer to the Live Streaming section first, then proceed with data saving.
...
When saving is complete, the Save Complete window appears. The raw data is saved as a `.dvs` file in the `RawDataSave` folder. This `.dvs` file can later be opened in File Playback mode.

#### 2.2 Saving Images or Videos
...
Note: Frame Generation Mode is compatible with both Live Streaming and File Playback.

### 4. How to Control Sensor Register Values in Real Time [In Progress]
